class UpdateWorkorderStatus < ActiveRecord::Migration
  def change
    ...
    total = 0
    total_record = Workorder.where("(status_id = 16 or status_id is null) and created_at <= ? and company_id IN(3,6,7,12,27)",'2013-01-01').count
    p "###### Total Record to updates #{total_record} Lote A"

    Workorder.where("(status_id = 16 or status_id is null) and created_at <= ? and company_id IN(3,6,7,12,27)",'2013-01-01').order("created_at ASC").find_in_batches do |batch|
