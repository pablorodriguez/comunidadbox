class MaterialServiceType < ActiveRecord::Base
  ...
  def self.materials company_id
    MaterialServiceType.all :include=>{:price_list_items_active=>{:price_list=>{}},:service_type=>{:companies=>{}}},
  ...
  def self.m(company_id,price_list_id,service_type_ids,material,page)
    ...
    if (page >=0)  
      return MaterialServiceType.paginate(:all,:per_page=>20,:page =>page,:select => select_str,:conditions=>['m.name LIKE ?',"%#{material}%"] ,:joins=>join_str,:order =>"st.name,m.name")  
    else
